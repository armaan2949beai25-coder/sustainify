...
// ===== POVERTY SIMULATION CALCULATOR =====
const budgetTotal = 150;
const expFood = document.getElementById("exp-food");
const expTransport = document.getElementById("exp-transport");
const expMedicine = document.getElementById("exp-medicine");
const expShelter = document.getElementById("exp-shelter");

const valFood = document.getElementById("val-food");
const valTransport = document.getElementById("val-transport");
const valMedicine = document.getElementById("val-medicine");
const valShelter = document.getElementById("val-shelter");

const remBalance = document.getElementById("rem-balance");
const diffLevel = document.getElementById("diff-level");
const awarenessMsg = document.getElementById("awareness-msg");
const calcBox = document.querySelector(".calc-box");
const calcResultH3 = document.querySelector(".calc-result h3");

function updateCalculator() {
  if (!expFood) return; // Guard clause in case elements aren't loaded

  const f = parseInt(expFood.value);
  const t = parseInt(expTransport.value);
  const m = parseInt(expMedicine.value);
  const s = parseInt(expShelter.value);

  valFood.textContent = f;
  valTransport.textContent = t;
  valMedicine.textContent = m;
  valShelter.textContent = s;

  const spent = f + t + m + s;
  const remaining = budgetTotal - spent;

  remBalance.textContent = remaining;

  if (remaining < 0) {
    calcBox.classList.add("warning-glow");
    calcResultH3.classList.add("deficit");
    diffLevel.className = "diff-level critical";
    diffLevel.innerHTML = "Difficulty: <span>Impossible</span>";
    awarenessMsg.textContent = "You have exceeded your daily budget. Many families still struggle to afford daily essentials.";
  } else if (remaining <= 30) {
    calcBox.classList.remove("warning-glow");
    calcResultH3.classList.remove("deficit");
    diffLevel.className = "diff-level hard";
    diffLevel.innerHTML = "Difficulty: <span>Extremely Hard</span>";
    awarenessMsg.textContent = "Barely surviving. Even basic healthcare can become unaffordable.";
  } else {
    calcBox.classList.remove("warning-glow");
    calcResultH3.classList.remove("deficit");
    diffLevel.className = "diff-level";
    diffLevel.innerHTML = "Difficulty: <span>Manageable (Barely)</span>";
    awarenessMsg.textContent = "Allocate your budget wisely. Remember, ₹150 must cover everything.";
  }
}

if (expFood) {
  [expFood, expTransport, expMedicine, expShelter].forEach(input => {
